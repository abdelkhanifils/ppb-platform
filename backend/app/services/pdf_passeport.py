...
TRADUCTIONS_AR: dict[str, str] = {
    ...
    "Livestock covered by this passport: from 1 to 50 head": "الماشية المشمولة بجواز السفر هذا: من 1 إلى 50 رأسًا",
...
TRADUCTIONS_ES: dict[str, str] = {
    ...
    "Livestock covered by this passport: from 1 to 50 head": "Ganado cubierto por este pasaporte: de 1 a 50 cabezas",
...
def _entete_bilingue(texte_fr: str, langue: str, style_base: ParagraphStyle = S_TABLE_ENTETE) -> Paragraph:
    ...
    if langue == "FR/AR" and POLICE_ARABE_DISPONIBLE and texte_fr in TRADUCTIONS_AR:
        ...
        return Paragraph(f"{texte_fr} <font face='{NOM_POLICE_ARABE}' size=10>{texte_ar}</font>", style_base)
...
def _table_composition_troupeau(langue: str = "FR/EN") -> Table:
    ...
    def _entete_bilingue_etroite(texte_fr: str) -> Paragraph:
        ...
        if langue == "FR/AR" and POLICE_ARABE_DISPONIBLE and texte_fr in TRADUCTIONS_AR:
            texte_ar = preparer_texte_arabe(TRADUCTIONS_AR[texte_fr])
            return Paragraph(f"{texte_fr}<br/><font face='{NOM_POLICE_ARABE}' size=9.5>{texte_ar}</font>", S_TABLE_ENTETE)
        if langue == "FR/EN" and texte_fr in TRADUCTIONS_EN:
