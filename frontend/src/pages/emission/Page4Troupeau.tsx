...
const ESPECES: { valeur: EspeceTroupeau; cle: string }[] = [
  { valeur: "bovin", cle: "page4.bovins" },
  { valeur: "ovin", cle: "page4.ovins" },
  { valeur: "caprin", cle: "page4.caprins" },
  { valeur: "camelin", cle: "page4.camelins" },
  { valeur: "autre", cle: "page4.autres" },
];
...
const MALADIES: { valeur: MaladieControlee; cle: string }[] = [
  { valeur: "peste_petits_ruminants", cle: "page4.peste_petits_ruminants" },
  { valeur: "peripneumonie_contagieuse", cle: "page4.peripneumonie" },
  { valeur: "charbon", cle: "page4.charbon" },
  { valeur: "trypanosomiase", cle: "page4.trypanosomiase" },
];
...
export default function Page4Troupeau({ passeportId, onValidee }: Page4Props) {
  ...
  const soumettre = async () => {
    ...
    if (effectifs.every((e) => e.nombre_total === 0)) {
      setErreursDynamiques({ _global: t("page4.effectif_vide") });
      return;
    }
  ...
  return (
    <div className="space-y-6">
      <h2 className="text-base font-semibold text-gray-900">{t("page4.titre")}</h2>

      <CapturePhotoOcr passeportId={passeportId} pageNum={4} onSuggestion={(c) => appliquerSuggestion(c as ChampsOcrPage4)} />

      <section className="space-y-3 rounded-lg border border-gray-200 bg-white p-4">
        <h3 className="text-sm font-semibold text-gray-800">{t("page4.composition")}</h3>
        ...
        {effectifs.map((effectif, index) => (
          <div key={index} className="grid grid-cols-12 items-end gap-2 border-b border-gray-100 pb-3 last:border-0">
            <div className="col-span-3">
              <label className="mb-1 block text-xs text-gray-500">{t("page4.espece")}</label>
              <select
            ...
            <ChampNombre label={t("page4.males")} valeur={effectif.nombre_males} onChange={(v) => majEffectif(index, "nombre_males", v)} />
            <ChampNombre label={t("page4.femelles_jeunes")} valeur={effectif.nombre_femelles_jeunes} onChange={(v) => majEffectif(index, "nombre_femelles_jeunes", v)} />
            <ChampNombre label={t("page4.femelles_adultes")} valeur={effectif.nombre_femelles_adultes} onChange={(v) => majEffectif(index, "nombre_femelles_adultes", v)} />
            <div className="col-span-2">
              <label className="mb-1 block text-xs text-gray-500">{t("page4.total")}</label>
        ...
        <button onClick={ajouterLigneEspece} className="flex items-center gap-1 text-sm text-cebevirha hover:underline">
          <Plus size={14} /> {t("page4.ajouter_espece")}
        </button>
      ...
      <section className="space-y-3 rounded-lg border border-gray-200 bg-white p-4">
        <h3 className="text-sm font-semibold text-gray-800">{t("page4.vaccinations")}</h3>
        {MALADIES.map((maladie) => {
          ...
          return (
            <div key={maladie.valeur} className="grid grid-cols-3 items-end gap-2">
              <p className="col-span-1 text-sm text-gray-700">{t(maladie.cle)}</p>
      ...
      {schemaTroupeau && schemaTroupeau.champs.length > 0 && (
        <section className="space-y-3 rounded-lg border border-gray-200 bg-white p-4">
          <h3 className="text-sm font-semibold text-gray-800">{t("page4.complementaires")}</h3>
          <FormulaireDynamique
      ...
      >
        {enCours ? t("page4.validation") : t("page4.valider")}
      </button>
    </div>
  );
}
