/**
 * Drapeaux des 6 pays CEMAC en SVG intégré — voir
 * frontend/src/components/DrapeauPays.tsx (même logique et mêmes formes,
 * dupliquées ici : les emojis de drapeau ne s'affichent pas comme des
 * drapeaux sur de nombreuses configurations Windows, confirmé en test
 * réel — la police système affiche les deux lettres du code pays au lieu
 * du drapeau composé).
 */
...
/** Icône de drapeau pour un code pays à 3 lettres — carré gris discret si
 * le code n'est pas reconnu (ne casse jamais l'affichage d'une liste). */
export function DrapeauPays({ codeIso, className, style }: { codeIso: string | undefined | null } & ProprietesDrapeau) {
  ...
  if (!Composant) {
    return (
      <span
        className={className}
        style={{ ...BASE, ...style, display: 'inline-block', background: '#e5e7eb', borderRadius: 2 }}
        aria-hidden="true"
      />
    );
  }
