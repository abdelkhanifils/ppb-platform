...
const DICO: Record<string, Entree> = {
  ...
  'reglages.poste_emission': { fr: 'Poste d\u2019émission', en: 'Emission post', es: 'Puesto de emisión', ar: 'مركز الإصدار' },
  'reglages.poste_emission_aide': { fr: 'Sert de lieu de vaccination par défaut — modifiable au cas par cas si la vaccination a eu lieu ailleurs.', en: 'Used as the default vaccination location — editable case by case if the vaccination took place elsewhere.', es: 'Se usa como lugar de vacunación por defecto — editable caso por caso si la vacunación tuvo lugar en otro sitio.', ar: 'يُستخدم كموقع افتراضي للتلقيح — قابل للتعديل حالة بحالة إذا جرى التلقيح في مكان آخر.' },
  'reglages.poste_emission_indisponible': { fr: 'Liste des postes indisponible pour le moment (nécessite une connexion).', en: 'List of posts unavailable right now (requires a connection).', es: 'Lista de puestos no disponible por el momento (requiere conexión).', ar: 'قائمة المراكز غير متاحة حاليًا (تتطلب اتصالاً).' },
  ...
  'reglages.purge_titre': { fr: 'Réinitialiser les données de test', en: 'Reset test data', es: 'Restablecer los datos de prueba', ar: 'إعادة تعيين بيانات الاختبار' },
  'reglages.purge_aide': { fr: 'Efface les passeports téléchargés et les émissions de cet appareil (déjà synchronisées ou non) — avant une démonstration ou la mise en exploitation réelle. Vous serez déconnecté(e) ; le mode hors-ligne n’est pas affecté, un nouveau téléchargement se fera à la prochaine connexion.', en: 'Erases downloaded passports and emissions on this device (synced or not) — before a demo or going live. You will be signed out; offline mode itself is unaffected, a fresh download will happen on next sign-in.', es: 'Borra los pasaportes descargados y las emisiones de este dispositivo (sincronizadas o no) — antes de una demostración o la puesta en explotación real. Se cerrará su sesión; el modo sin conexión no se ve afectado, se realizará una nueva descarga en el próximo inicio de sesión.', ar: 'يمسح جوازات السفر المحمَّلة والإصدارات على هذا الجهاز (المُزامَنة أو غيرها) — قبل عرض تجريبي أو الانتقال إلى الاستغلال الفعلي. سيتم تسجيل خروجك؛ وضع عدم الاتصال نفسه لن يتأثر، وستتم عملية تحميل جديدة عند تسجيل الدخول التالي.' },
  'reglages.purge_confirmation': { fr: 'Cette action est irréversible sur cet appareil, y compris pour des émissions pas encore envoyées au serveur. Continuer ?', en: 'This action is irreversible on this device, including for emissions not yet sent to the server. Continue?', es: 'Esta acción es irreversible en este dispositivo, incluidas las emisiones aún no enviadas al servidor. ¿Continuar?', ar: 'هذا الإجراء لا يمكن التراجع عنه على هذا الجهاز، بما في ذلك الإصدارات التي لم تُرسل بعد إلى الخادم. المتابعة؟' },
  'reglages.purge_confirmer': { fr: 'Oui, tout effacer', en: 'Yes, erase everything', es: 'Sí, borrar todo', ar: 'نعم، امسح الكل' },
  ...
  'etape2.revoque': { fr: 'Ce passeport a été RÉVOQUÉ — il ne peut plus être utilisé. Signalez-le à votre hiérarchie.', en: 'This passport has been REVOKED — it can no longer be used. Report it to your supervisor.', es: 'Este pasaporte ha sido REVOCADO — ya no puede utilizarse. Infórmelo a su superior.', ar: 'تم إلغاء جواز السفر هذا — لا يمكن استخدامه بعد الآن. أبلغ رئيسك.' },
